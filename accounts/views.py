    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, "your account was created successfuly"
            )
            return redirect("/")
        else:
...
def change_password(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                if user.check_password(old_password):
                    if (
                        ...
                        messages.add_message(
                            request, messages.ERROR, "your new password are not same "
                        )
                        return redirect("accounts:change_password")
                else:
                    messages.add_message(
                        request, messages.ERROR, "your old password  is not valid"
                    )
                    return redirect("accounts:change_password")
            ...
            return render(
                request, "registrations/change_password.html", context=context
            )
    else:
        messages.add_message(request, messages.ERROR, "input data is not valid")
        return redirect("accounts:login")
...
def reset_password_confirm(request, token):
    if request.method == "POST":
        ...
        return render(
            request, "registrations/reset_password_confirm.html", context=context
        )
